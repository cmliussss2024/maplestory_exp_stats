    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        for index, (caption, var) in enumerate(rows):
            ...
            row.pack(fill="x", pady=(0, Spacing.info_cell_spacing) if index < last else 0)
            if caption == "状态":
                self.status_label = row.value_label

        self.retry_row = RetryRowCell(self.content, on_retry=on_retry)
        self.retry_row.pack(fill="x", pady=(Spacing.info_cell_spacing, 0))

        bg = str(self.content.cget("bg"))
        self.image_canvas = tk.Canvas(
            self.content,
            width=Spacing.preview_width,
            height=Spacing.preview_height,
            bg=bg,
            highlightthickness=0,
            bd=0,
        )
        self.image_canvas.pack(pady=(Spacing.retry_preview_spacing, 0))
    ...
    def set_image(self, photo: tk.PhotoImage) -> None:
        if self._image_id is None:
            self._image_id = self.image_canvas.create_image(
                Spacing.preview_width // 2,
                Spacing.preview_height // 2,
                image=photo,
                anchor="center",
            )
        else:
